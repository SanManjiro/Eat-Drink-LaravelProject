@extends('layouts.app')

@section('title', 'Inscription')

@section('content')
    <div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-black to-gray-900">
        <div class="max-w-md w-full space-y-8">
            <div class="card p-8 animate-fade-in-up">
                <!-- Header -->
                <div class="text-center mb-8">
                    <div class="flex items-center justify-center mb-6">
                        <div class="w-16 h-16 bg-white rounded-3xl flex items-center justify-center shadow-glow">
                            <i data-lucide="user-plus" class="w-8 h-8 text-black"></i>
                        </div>
                    </div>
                    <h2 class="text-3xl font-black text-white mb-2">Demande de Stand</h2>
                    <p class="text-white/60">Rejoignez notre communauté d'entrepreneurs</p>
                </div>

                <!-- Message d'information -->
                <div class="mb-6 p-4 rounded-2xl bg-blue-500/20 border border-blue-500/30">
                    <div class="flex items-start space-x-2 text-blue-300">
                        <i data-lucide="info" class="w-5 h-5 mt-0.5"></i>
                        <div>
                            <p class="font-medium">Information importante</p>
                            <p class="text-sm text-blue-300/80 mt-1">
                                Votre demande sera examinée par notre équipe. Vous recevrez une notification une fois votre compte approuvé.
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Affichage des erreurs -->
                @if ($errors->any())
                    <div class="mb-6 p-4 rounded-2xl bg-red-500/20 border border-red-500/30">
                        <div class="flex items-center space-x-2 text-red-300">
                            <i data-lucide="alert-circle" class="w-5 h-5"></i>
                            <span class="font-medium">Erreurs dans le formulaire</span>
                        </div>
                        <ul class="mt-2 text-red-300/80 text-sm space-y-1">
                            @foreach ($errors->all() as $error)
                                <li>{{ $error }}</li>
                            @endforeach
                        </ul>
                    </div>
                @endif

                <!-- Formulaire -->
                <form method="POST" action="{{ route('register') }}" class="space-y-6">
                    @csrf

                    <!-- Nom de l'entreprise -->
                    <div>
                        <label for="nom_entreprise" class="form-label">
                            <i data-lucide="building" class="w-5 h-5 inline mr-2"></i>
                            Nom de l'Entreprise
                        </label>
                        <input type="text"
                               id="nom_entreprise"
                               name="nom_entreprise"
                               value="{{ old('nom_entreprise') }}"
                               required
                               autocomplete="organization"
                               class="form-input"
                               placeholder="Le nom de votre entreprise">
                    </div>

                    <!-- Email -->
                    <div>
                        <label for="email" class="form-label">
                            <i data-lucide="mail" class="w-5 h-5 inline mr-2"></i>
                            Adresse Email
                        </label>
                        <input type="email"
                               id="email"
                               name="email"
                               value="{{ old('email') }}"
                               required
                               autocomplete="email"
                               class="form-input"
                               placeholder="votre@email.com">
                    </div>

                    <!-- Mot de passe -->
                    <div>
                        <label for="password" class="form-label">
                            <i data-lucide="lock" class="w-5 h-5 inline mr-2"></i>
                            Mot de Passe
                        </label>
                        <input type="password"
                               id="password"
                               name="password"
                               required
                               autocomplete="new-password"
                               class="form-input"
                               placeholder="Minimum 8 caractères">
                    </div>

                    <!-- Confirmation du mot de passe -->
                    <div>
                        <label for="password_confirmation" class="form-label">
                            <i data-lucide="lock" class="w-5 h-5 inline mr-2"></i>
                            Confirmer le Mot de Passe
                        </label>
                        <input type="password"
                               id="password_confirmation"
                               name="password_confirmation"
                               required
                               autocomplete="new-password"
                               class="form-input"
                               placeholder="Confirmez votre mot de passe">
                    </div>

                    <!-- Description du stand -->
                    <div>
                        <label for="description" class="form-label">
                            <i data-lucide="file-text" class="w-5 h-5 inline mr-2"></i>
                            Description de votre Stand
                        </label>
                        <textarea id="description"
                                  name="description"
                                  rows="4"
                                  class="form-textarea"
                                  placeholder="Décrivez votre activité, vos spécialités...">{{ old('description') }}</textarea>
                    </div>

                    <!-- Conditions d'utilisation -->
                    <div class="flex items-start">
                        <input type="checkbox"
                               name="terms"
                               required
                               class="mt-1 rounded border-white/20 bg-white/5 text-white focus:ring-white/30 focus:ring-offset-0">
                        <label class="ml-3 text-white/70 text-sm leading-relaxed">
                            J'accepte les
                            <a href="#" class="text-white hover:text-gray-300 underline">conditions d'utilisation</a>
                            et la
                            <a href="#" class="text-white hover:text-gray-300 underline">politique de confidentialité</a>
                        </label>
                    </div>

                    <!-- Bouton de soumission -->
                    <button type="submit" class="w-full btn-primary justify-center py-4">
                        <i data-lucide="send" class="w-5 h-5 mr-2"></i>
                        Soumettre ma Demande
                    </button>
                </form>

                <!-- Lien de connexion -->
                <div class="mt-8 text-center">
                    <p class="text-white/60">
                        Déjà un compte ?
                        <a href="{{ route('login') }}"
                           class="text-white hover:text-gray-300 font-medium transition-colors duration-300">
                            Connectez-vous ici
                        </a>
                    </p>
                </div>
            </div>
        </div>
    </div>
@endsection
